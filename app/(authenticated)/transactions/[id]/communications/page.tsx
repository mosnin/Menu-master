...
const approvalStatusConfig: Record<string, { label: string; className: string }> = {
  pending: { label: 'Awaiting Approval', className: 'text-amber-600 bg-amber-50/60' },
  approved: { label: 'Approved', className: 'text-emerald-600 bg-emerald-50/60' },
  rejected: { label: 'Rejected', className: 'text-red-600 bg-red-50/60' },
};
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border overflow-hidden">
        <div className="grid grid-cols-[1fr_1fr_140px_120px_100px] gap-2 px-5 py-3 bg-muted/30 text-[11px] font-semibold text-muted-foreground/70 uppercase tracking-wider border-b">
          <span>Recipient</span>
          <span>Subject</span>
          <span>Status</span>
          <span>Approval</span>
          ...
          <div className="divide-y">
            {filtered.map((comm) => {
              ...
              return (
                <div key={comm.id}>
                  <div
                    className="grid grid-cols-[1fr_1fr_140px_120px_100px] gap-2 px-5 py-3.5 items-center cursor-pointer hover:bg-muted/20 transition-colors duration-200"
                  ...
                  >
                    ...
                    <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-medium w-fit ${
                      comm.status === 'sent' ? 'bg-emerald-50/70 text-emerald-600' :
                      comm.status === 'failed' ? 'bg-red-50/70 text-red-600' :
                      comm.status === 'pending_approval' ? 'bg-amber-50/70 text-amber-600' :
                      comm.status === 'approved' ? 'bg-blue-50/70 text-blue-600' :
                    ...
                    <div>
                      {approval ? (
                        <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-medium ${approval.className}`}>
                          {approval.label}
                        </span>
                      ) : (
                  ...
                  {isExpanded && (
                    <div className="px-6 pb-4 space-y-3 border-t bg-muted/[0.04]">
                      <div className="pt-4">
                        <p className="text-[11px] font-semibold text-muted-foreground/70 uppercase tracking-widest mb-2">Preview</p>
                        <p className="text-sm text-muted-foreground/80 bg-background rounded-lg border border-border/60 p-4 leading-relaxed">
                          {comm.body_preview}
                        </p>
                      </div>
    ...
    <div className="space-y-8">
      ...
      {communications.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center rounded-xl border">
          <Mail className="h-8 w-8 text-muted-foreground/40 mb-4" />
          <p className="text-sm font-medium text-muted-foreground">No communications yet</p>
